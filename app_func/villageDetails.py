...
def getAll():
    ...
    if topic == "village":
        ...
        mycursor.execute(
            "SELECT nameChineseCharacters_村名汉字 FROM village_村 WHERE gazetteerId_村志代码={}".format(village_id))
  ...
  WHERE p.gazetteerId_村志代码 ={}".format(village_id))
    ...
    if count_valid == len(table["data"]):
        ...
        # write into csv
        path = os.getcwd()
        f = open(os.path.join(path, "app_func", "single_csv", "{}_{}.csv".format(village_id, topic)), 'w',
                 encoding='utf-8')
...
# @village_blueprint.route("/getAllName", methods=["POST"], strict_slashes=False)
# def getAllName:
...
#   table2func[mecialTypelist] = getEthnicgroups
